...
export function DocumentActionDetails({children}: {children: React.ReactNode}) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('document-actions-open', close);
    return () => window.removeEventListener('document-actions-open', close);
  }, []);
  ...
  return (
    <details
      ...
      onToggle={() => {
        if (detailsRef.current?.open) {
          window.dispatchEvent(new Event('document-actions-open'));
    ...
    >
      ...
      <div
        className="fixed z-[9999] w-44 rounded-lg border border-[var(--line-soft)] bg-white p-1 text-left text-sm shadow-xl"
