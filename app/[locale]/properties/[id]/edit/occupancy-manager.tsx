    ...
    <div className="grid gap-5" ref={managerRef}>
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex min-h-20 cursor-pointer items-center justify-between rounded-lg border border-[var(--accent)] bg-[#f5faf8] px-4">
          <span>
            <span className="block font-semibold text-[var(--accent)]">{t('vacant')}</span>
            <span className="text-sm text-[var(--muted)]">{t('vacantDescription')}</span>
          </span>
        ...
        <label className="flex min-h-20 cursor-pointer items-center justify-between rounded-lg border border-[var(--line-soft)] px-4">
          <span>
            <span className="block font-semibold">{t('rented')}</span>
            <span className="text-sm text-[var(--muted)]">{t('rentedDescription')}</span>
          </span>
...
export function LeaseTerminationManager({leases, locale, propertyId, returnTo}: {leases: ActiveLease[]; locale: string; propertyId: string; returnTo?: string}) {
  ...
  return (
    <div className="grid gap-3 rounded-lg border border-[var(--line-soft)] bg-white p-4">
      <h3 className="text-sm font-semibold">{t('existingTenants')}</h3>
      {leases.map((lease) => (
        <form
          ...
          key={lease.id}
          onSubmit={(event) => {
            if (!window.confirm(t('confirmEndLease'))) {
              event.preventDefault();
            }
          }}
        >
          ...
          <div>
            <p className="font-medium">{lease.tenants?.full_name ?? t('tenant')}</p>
            <p className="mt-1 text-sm text-[var(--muted)]">{t('since', {date: isoDateToDisplay(lease.start_date)})}</p>
          </div>
