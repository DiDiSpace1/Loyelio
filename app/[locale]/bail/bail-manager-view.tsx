...
import {OccupancyManager} from '../properties/[id]/edit/occupancy-manager';
    ...
    <AppShell>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{selectedProperty?.name ?? t('selectPropertyToManage')}</p>
        </div>
        {selectedProperty ? (
      ...
      <section className="mt-8 rounded-lg border border-[var(--line-soft)] bg-white p-5 shadow-sm">
        <div className="mb-5">
        ...
        {selectedProperty ? (
          <>
            ...
            <form action={assignPropertyTenantsAction} className="grid gap-5">
